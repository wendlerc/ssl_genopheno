...
project = "genotype_supervised_biggerdata_l1eq0"
pp = "datasets/genotype/cas9/cas9_pairs_%s.csv"
...
for seed in (42 + np.arange(10)):
    ...
    print(" -W 4:00 python project/genotype/regression.py --l1_coef 0 ", end="")
    print(" --wandb_project %s"%project, end="")
    print(" --path_pattern %s"%pp, end="")
    print(" --wandb_name trained_%d --seed %d --group trained"%(seed, seed))
    
    print("bsub -o /cluster/scratch/wendlerc/lsfcsss/ -G ls_krausea", end="")
    print(" -n 12 -R \"rusage[mem=1000, ngpus_excl_p=1]\" -R \"select[gpu_mtotal0>=10000]\"", end="")
    print(" -W 4:00 python project/genotype/regression.py --l1_coef 0 --freeze", end="") 
    print(" --wandb_project %s"%project, end="")
    print(" --path_pattern %s"%pp, end="")
