...
for seed in (42 + np.arange(10)):
    print("bsub -o /cluster/scratch/wendlerc/lsfcsss/ -G es_puesch", end="")
    print(" -n 12 -R \"rusage[mem=1000, ngpus_excl_p=1]\" -R \"select[gpu_mtotal0>=10000]\"", end="")
    print(" -W 4:00 python project/genotype/regression.py --l1_coef 50 ", end="")
    print(" --wandb_project %s"%project, end="")
    print(" --use_subset --random --train_fraction 0.25", end="")
    print(" --wandb_name trained_%d --seed %d --group trained"%(seed, seed))
    
    print("bsub -o /cluster/scratch/wendlerc/lsfcsss/ -G es_puesch", end="")
    print(" -n 12 -R \"rusage[mem=1000, ngpus_excl_p=1]\" -R \"select[gpu_mtotal0>=10000]\"", end="")
    print(" -W 4:00 python project/genotype/regression.py --l1_coef 0.009 --freeze", end="") 
    print(" --wandb_project %s"%project, end="")
    print(" --use_subset --random --train_fraction 0.25", end="")
