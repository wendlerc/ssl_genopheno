...
from pretrain_vicreg import CompressiveSensingPretraining
...
def main():
    ...
    parser.add_argument('--artifact_dir', default="./artifacts", type=str)
    # datamodule args
    ...
    if args.wandb_pretrained is None:
        ...
        pmodel = CompressiveSensingPretraining(encoder, use_bn=args.normalize)
        ...
        pmodel = CompressiveSensingPretraining.load_from_checkpoint('%s/%s/%s'%(args.artifact_dir, args.wandb_pretrained, mkey.split('/')[-1]), 
                                                                    use_bn=args.normalize, encoder=encoder)
        model = GenotypeRegression(pmodel, lr=args.lr, 
                                   beta1=args.beta1, 
                                   beta2=args.beta2,
                                   factor=args.factor,
