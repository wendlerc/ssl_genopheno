...
def augment(data, px=0.1, pe=0.5): 
    masked_data = copy.deepcopy(data)
    k = masked_data.x.shape[1]
    e = masked_data.edge_index.shape[1] # COO format [[...........], 
                                        #             [...........]]

    probs = (1 - px) * torch.ones(k, device=device)
    mask = torch.bernoulli(probs)
    masked_data.x = masked_data.x * mask

    probs = (1 - pe) * torch.ones(e, device=device)
    mask = torch.bernoulli(probs).long()
    masked_data.edge_index = masked_data.edge_index[:, mask]

    return masked_data

def dataset_subset(data, num_train_per_class = 1 / 5, runs=20):
    '''
        data: WikiCS dataset with 20 different train splits where each split contains 5% of the samples of each class
        num_train_per_class: the fraction of the 5% of the per class train samples to be selected at random
    '''
    N = data.x.shape[0]

    train_mask = torch.zeros(N, runs)
    classes = max(data.y) + 1
    for j in range(runs):
        for i in range(classes):
            indices = torch.where((data.y == i) & (data.train_mask[:, j] == 1))
            total_of_class = indices[0].shape[0]
            perm = torch.randperm(total_of_class)
            num_of_subset = int(num_train_per_class * total_of_class)
            idx = indices[0][perm[:num_of_subset]]
            train_mask[idx, j] = 1 # set to 1 the corresponding train_samples
...
def load_dataset(name):
    ...
    if name == "WikiCS":
        dataset = WikiCS('.', is_undirected=True, transform=feature_norm)

    elif (name in ['Computers', 'Photo']):
        dataset = Amazon('.', name=name, transform=feature_norm, pre_transform=create_masks)

    elif (name in ['CS', 'Physics']):
        dataset = Coauthor('.', name=name, transform=feature_norm, pre_transform=create_masks)
...
def show_mat(model, data, MODEL, DATASET, embedding, emb_dim):
    features = model(data)
    mat = features.T @ features / (features.shape[0])
    mat = mat.cpu().detach().numpy()
    d = mat.shape[0]

    diag = sum([mat[i,i] for i in range(d)])
    ratio = np.sum(mat) / diag - 1

    plt.figure()
    plt.axis('off')
    plt.grid(False)
    plt.imshow(mat, cmap='gray_r')
    plt.title('$\Phi^T\Phi$ ' + ' off diag / diag = {:.3f}'.format(ratio))
    plt.savefig('results/Orthogonality_inspection_{}_{}_embedding_{}_emb_dim_{}.png'
                    .format(MODEL, DATASET, embedding, emb_dim))